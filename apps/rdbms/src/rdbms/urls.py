...
# APIs
urlpatterns += [
  url(r'^api/servers/?$', api.servers, name='api_servers'),
  url(r'^api/servers/(?P<server>\w+)/databases/?$', api.databases, name='api_databases'),
  url(r'^api/servers/(?P<server>\w+)/databases/(?P<database>.+?)/tables/?$', api.tables, name='api_tables'),
  url(r'^api/servers/(?P<server>\w+)/databases/(?P<database>.+?)/tables/(?P<table>\w+)/columns/?$', api.columns, name='api_columns'),
  url(r'^api/query/((?P<design_id>\d+)/?)?$', api.save_query, name='api_save_query'),
  url(r'^api/query/(?P<design_id>\d+)/get$', api.fetch_saved_query, name='api_fetch_saved_query'),
  url(r'^api/execute/(?P<design_id>\d+)?$', api.execute_query, name='api_execute_query'),
  url(r'^api/explain/?$', api.explain_query, name='api_explain_query'),
  url(r'^api/results/(?P<id>\d+)/(?P<first_row>\d+)$', api.fetch_results, name='api_fetch_results')
]
